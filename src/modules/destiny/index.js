...
/**
 * Simple async function to make a get request to the Bungie API. Returns a json
 * object containing the response data.
 * @param {string} path endpoint to send to Bungie API
 * @return {JSON} json data containing response.
 */
...
module.exports = {
   ...
   * @return {JSON} json data with response.
   */
   ...
   * @param {string} directorActivityHash unique hash representing a specific
   * activity type
   * @return {JSON} json data with response
   */
  getActivityDefinition: async function(directorActivityHash) {
    if (Number.isInteger(directorActivityHash)) {
      return _apiGet(
          '/Platform/Destiny2/Manifest/DestinyActivityDefinition/' +
          directorActivityHash,
      );
    } else {
      logger.logError(
          `director_activity_hash was not an integer (${directorActivityHash})`,
      );
      return;
    }
  },
   ...
   * @return {JSON} profile from Bungie API
   */
  getAccountStats: async function(membershipType, membershipId) {
    ...
    return _apiGet(
        `/Platform/Destiny2/${membershipType}/Account/${membershipId}/Stats/`,
    );
  },
   ...
   * @return {JSON} json data given a specific page.
   */
  searchGlobalName: async function(displayName, page) {
    ...
    const jsonData = {'displayNamePrefix': displayName};
    return _apiPost(
  ...
  /**
   * return a destiny profile based on included components.
   * @param {string} membershipType describe the type of platform the membership
   * exists on
   * @param {string} membershipId unique membership hash
   * @param {Array} components list of components to send to the request.
   * @return {JSON} a destiny profile with included components
   */
